...
void WINAPI LIBRARYENTRY( unsigned int ) 
{
    ...
    IBodyCollection* bodys = part->BodyCollection();
    for( long i = 0; i < bodys->GetCount(); i++ )
    {
        IBody* body = bodys->GetByIndex( i );
        ss << "Body number " << i << ":" << std::endl << "edge lengths:"<<std::endl;    
        IFaceCollection* faces = body->FaceCollection();
        for( long j = 0; j < faces->GetCount(); j++ )
        {
            IEdgeCollection* edges = faces->GetByIndex(j)->EdgeCollection();
            for( long k = 0; k < edges->GetCount(); k++ )
            {
                IEdgeDefinition* e = edges->GetByIndex( k );
                double length = e->GetLength(0x3);
                ss << length << "m" << std::endl;
            }
        }
    }
